...
export default function PollPage() {
  ...
  const submit = () => { dispatch(answerQuestion({ authedUser, qid: id, answer: selected })); message.success('Vote saved'); };
  return
    <Card className="section-card" title="Would you rather?">
      <UserPreview user={users[question.author]} />
      <Typography.Divider />
      <Responses question={question} selected={selected} onSelect={setSelected} showResults={answered} />{!answered && <Button type="primary" className="mt" disabled={!selected} onClick={submit}>Submit vote</Button>}
  </Card>;
}
